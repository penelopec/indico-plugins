...
from indico.util.string import encode_if_unicodeimport string
...
class JSONGen:
    ...
    def initJson(self):
        self.json=["""<?xml version="1.0" encoding="UTF-8"?>\n"""]

    def getJson(self):
        return "".join(self.json)
    ...
    def openTag(self, name, listAttrib=[], single=False):
        ...
        for i in range(0, self.indent):
            self.json.append(" ")
        self.json.append("<" + name + LAtt + ">")
        if not single:
            self.json.append(self.escapeString("\r\n"))
    ...
    def writeText(self,text, single=False):
        ...
        if text != "":
            ...
            self.json.append(self.escapeString(text))
        if not single:
            self.json.append( self.escapeString("\r\n"))
    ...
    def writeJSON(self,text):
        #add already well-formated text
        self.json.append( text )

    def cleanText(self, text):
        # clean the text from illegal JSON characters
        cm = []
        for c in range(256):
...
class outputGenerator(object):
    def __init__(self, user, JG=None):
        self.__user = user
        if JG is not None:
            self._JSONGen = JG
        else:
            self._JSONGen = JSONGen()
        self.text = ""
        self.time_JSON = 0
        self.time_HTML = 0
    ...
    def _generateACLDatafield(self, eType, memberList, objId, out):
        """
        Generates a specific JSON field containing the ACL
        """
        if eType:
    ...
    def confToJSON(self, event, includeSession=1, includeContribution=1, includeMaterial=1, out=None):
        if not out:
            out = self._JSONGen
        temp = JSONGen(init=False)
        self._event_to_json(event, includeSession, includeContribution, includeMaterial, out=temp)
        json = temp.getJson()
        out.writeJSON(json)
    ...
    def _event_to_json(self, event, includeSession=1, includeContribution=1, includeMaterial=1, out=None):
        if not out:
            out = self._JSONGen
        ...
        if includeMaterial:
            self.materialToJSON(event, out=out)
        #out.closeTag("datafield")

        if event.note and not event.note.is_deleted:
            self.noteToJSON(event.note, out=out)
    ...
    def contribToJSON(self, contrib, includeMaterial=1, out=None):
        if not out:
            out = self._JSONGen
        temp = JSONGen(init=False)
        self._contrib_to_json(contrib, includeMaterial, out=temp)
        json = temp.getJson()
        out.writeJSON(json)

    def _contrib_to_json(self, contrib, include_material=1, out=None):
        if not out:
            out = self._JSONGen
        ...
        if include_material:
            self.materialToJSON(contrib, out=out)

        if contrib.note and not contrib.note.is_deleted:
            self.noteToJSON(contrib.note, out=out)
    ...
    def subContribToJSON(self,subCont,includeMaterial=1, out=None):
        if not out:
            out = self._JSONGen
        temp = JSONGen(init=False)
        self._subcontrib_to_json(subCont,includeMaterial, out=temp)
        json = temp.getJson()
        out.writeJSON(json)

    def _subcontrib_to_json(self, subcontrib, includeMaterial=1, out=None):
        if not out:
            out = self._JSONGen
    ...
    #
        ...
        if includeMaterial:
            self.materialToJSON(subcontrib, out=out)

        if subcontrib.note and not subcontrib.note.is_deleted:
            self.noteToJSON(subcontrib.note, out=out)
    ...
    def materialToJSON(self, obj, out=None):
        if not out:
            out = self._JSONGen
        for attachment in (Attachment.find(~AttachmentFolder.is_deleted, AttachmentFolder.object == obj,
            ...
            if attachment.can_access(self.__user):
                self.resourceToJSON(attachment, out)
    ...
    def resourceToJSON(self, res, out=None):
        if not out:
            out = self._JSONGen
        if res.type == AttachmentType.file:
            self.resourceFileToJSON(res, out=out)
        else:
            self.resourceLinkToJSON(res, out=out)
    ...
    def resourceLinkToJSON(self, attachment, out=None):
        if not out:
            out = self._JSONGen
    ...
    def resourceFileToJSON(self, attachment, out=None):
        if not out:
            out = self._JSONGen
    ...
    def noteToJSON(self, note, out=None):
        if not out:
            out = self._JSONGen
        out.openTag('datafield', [['tag', '856'], ['ind1', '4'], ['ind2', ' ']])

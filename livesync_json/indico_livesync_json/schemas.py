...
def _get_attachment_content(attachment):
    ...
    if attachment.type == AttachmentType.file:
        if DEBUG:
            import tika
            tika.initVM()
            parsedfile = parser.from_file(attachment.absolute_download_url)
        else:
            from indico_livesync_json.plugin import JsonLiveSyncPlugin
            parsedfile = parser.from_file(attachment.absolute_download_url, serverEndpoint=JsonLiveSyncPlugin.settings.get('tika_server')
...
class AttachmentSchema(mm.ModelSchema):
    ...
    $schema = mm.String(default='{}/{}'.format(JsonLiveSyncPlugin.settings.get("search_instance"),JsonLiveSyncPlugin.settings.get("attachments")))
...
class ContributionSchema(mm.ModelSchema):
    ...
    $schema = mm.String(default='{}/{}'.format(JsonLiveSyncPlugin.settings.get("search_instance"),JsonLiveSyncPlugin.settings.get("contributions")))
...
class SubContributionSchema(mm.ModelSchema):
    ...
    $schema = mm.String(default='{}/{}'.format(JsonLiveSyncPlugin.settings.get("search_instance"),JsonLiveSyncPlugin.settings.get("subcontributions")))
...
class EventNoteSchema(mm.ModelSchema):
    ...
    $schema = mm.String(default='{}/{}'.format(JsonLiveSyncPlugin.settings.get("search_instance"),JsonLiveSyncPlugin.settings.get("notes")))

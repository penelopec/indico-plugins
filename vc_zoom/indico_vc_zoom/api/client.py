...
from indico_vc_zoom.api.cache import ZeepCache
        ...
        while True:
            ...
            for room in response.room:
                if int(room.extension) == int(extension):
                    ZoomPlugin.logger.debug('Room: %s has been found.', room)
                    return room
                else:
                    ZoomPlugin.logger.debug('Dismissing room extension %s', room.extension)
                counter += 1

    @raises_api_error
    def get_room(self, zoom_id):
        return self.soap.getRoom(roomID=zoom_id)
